...
class MonitorWorker(QObject):
    ...
    def __init__(self, api_key, model_name, display_mode):
        super().__init__()
        self.api_key = api_key
        self.model_name = model_name
        self.display_mode = display_mode
        self.monitor = UsageMonitor(api_key, model_name, display_mode)
        self._running = True
    ...
    def update_params(self, api_key, model_name, display_mode):
        self.api_key = api_key
        self.model_name = model_name
        self.display_mode = display_mode
        self.monitor.update_params(api_key, model_name, display_mode)
...
class MiniMaxMonitor(QWidget):
    def __init__(self):
        ...
        self.display_mode = cfg.get("display_mode", "Used")
    ...
    def initUI(self):
        ...
        m_layout = QVBoxLayout(self.monitor_view)
        ...
        self.model_label.setStyleSheet(f"font-size: 10px; color: {ROSE_RED_LIGHT}; font-weight: bold;")
        m_footer.addWidget(self.rpm_label)
        m_footer.addStretch()
        ...
        c_layout = QVBoxLayout(self.config_view)
        c_layout.setContentsMargins(0, 4, 0, 4)
        c_layout.setSpacing(5)
        self.input_key = QLineEdit(self.api_key)
        ...
        self.combo_model.addItems(["MiniMax-M*", "speech-hd", "image-01", "music-2.5"])
        self.combo_model.setCurrentText(self.model_name)
        self.combo_model.setStyleSheet("background: rgba(255,255,255,20); color: white; border: 1px solid #444; min-height: 20px; border-radius: 4px;")
        
        mode_row = QHBoxLayout()
        mode_row.addWidget(QLabel("Mode", styleSheet="font-size: 9px; color: #AAA;"))
        self.combo_mode = QComboBox()
        self.combo_mode.addItems(["Used", "Remains"])
        self.combo_mode.setCurrentText(self.display_mode)
        self.combo_mode.setFixedHeight(20)
        mode_row.addWidget(self.combo_mode)
        
        opacity_row = QHBoxLayout()
        opacity_row.addWidget(QLabel("Opacity", styleSheet="font-size: 9px; color: #AAA;"))
        ...
        self.slider_opacity.valueChanged.connect(self.onOpacityChanged)
        opacity_row.addWidget(self.slider_opacity)
        
        self.config_msg = QLabel("")
        self.config_msg.setStyleSheet("font-size: 9px; color: #F44; font-weight: bold;")
    ...
    def updateStyle(self, hovered=False):
        ...
        self.container.setStyleSheet(f"""
            #MainContainer {{ background-color: rgba(20, 20, 20, {self.bg_opacity}); border: 2px solid {border_color}; border-radius: 12px; }}
            #HeaderBtn {{ background-color: transparent; color: white; border: none; font-size: 16px; font-weight: bold; }}
            ...
            QComboBox {{ background: rgba(255,255,255,20); color: white; border: 1px solid #444; border-radius: 4px; padding: 1px 3px; font-size: 9px; }}
            QComboBox QAbstractItemView {{ background-color: #1a1a1a; color: white; selection-background-color: {ROSE_RED}; outline: none; border: 1px solid {ROSE_RED}; }}
        """)
    ...
    def update_header_positions(self, hovered):
        ...
        target_x = w - 45 - self.interval_label.width() if hovered else w - self.interval_label.width()
        self.anim.stop()
        self.anim.setEndValue(QPoint(int(target_x), 0))
        self.anim.start()

    def enterEvent(self, event):
        self.updateStyle(hovered=True)
        self.sizegrip.setVisible(True)
        self.update_header_positions(True)
        super().enterEvent(event)

    def leaveEvent(self, event):
        self.updateStyle(hovered=False)
        self.sizegrip.setVisible(False)
    ...
    def persist_current_geometry(self):
        ...
        save_config(self.api_key, self.bg_opacity, self.model_name, self.last_geometry, self.display_mode)

    def save_config_action(self):
        key = self.input_key.text().strip()
        if not key:
            self.config_msg.setText("Empty Key!")
            return
        self.api_key = key
    ...
    def startWorker(self):
        ...
        if self.thread:
            ...
            self.thread.wait()
        self.thread = QThread()
        self.worker = MonitorWorker(self.api_key, self.model_name, self.display_mode)
        self.worker.moveToThread(self.thread)
        self.thread.started.connect(self.worker.run)
        self.worker.data_updated.connect(self.updateDisplay)
        self.worker.error_occurred.connect(self.handleError)
        self.thread.start()

    def updateDisplay(self, data):
        self.usage_label.setText(data["usage"])
    ...
    def handleError(self, msg):
        ...
        self.interval_label.setText(f"API Error")
